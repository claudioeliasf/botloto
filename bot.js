...
client.on("message", async (message) => {
    try {
        const mensagemRecebida = message.body.toLowerCase().trim();
        const isPrivado = !message.from.includes('@g.us');
        const isGrupo = message.from.includes('@g.us');

        console.log(`Mensagem recebida: "${mensagemRecebida}" | Privado: ${isPrivado} | Grupo: ${isGrupo}`);

        if (mensagemRecebida === "botloto") {
            const usuario = isGrupo ? message.author : message.from;
            console.log(`Comando "botloto" recebido ${isGrupo ? 'no grupo' : 'no privado'} por ${usuario}.`);
